...
function formatGaffe(gaffe: Record<string, any>): string {
  ...
  if (gaffe.featureTriggered === false) {
    parts.push(`featureTriggered: false`);
  } else if (Array.isArray(gaffe.featureTriggered) && gaffe.featureTriggered.length > 0) {
    parts.push(`featureTriggered: [${gaffe.featureTriggered.join(",")}]`);
  }
