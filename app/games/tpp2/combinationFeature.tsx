...
export default function CombinationFeature({
  ...
  const collectUsedMults = (g: ComboCell[][]): Set<string> => {
    ...
    g.flat().forEach(c => { if (c.type === "RED" && c.multiplier) s.add(c.multiplier); });
    return s;
  };

  const [isOpen,        setIsOpen]        = useState(true);
  const [grid,          setGrid]          = useState<ComboCell[][]>(() => seedGrid());
  ...
  // Features that can still be added via an upgrade coin (hidden once all 3 active).
  const upgradeTargets = onUpgrade ? availableUpgradeTargets(selectedFeatures) : [];
 
  // ── Re-seed when inputs change ────────────────────────────────────────────
  useEffect(() => {
    const g = seedGrid();
    zoneCounter.current = 0;
    // Rebuild zones for any carried PURPLE anchors sitting in unlocked rows.
  ...
  const buildCarried = (g: ComboCell[][]): CarriedCoin[] => {
  ...
  const handleCellClick = (r: number, c: number) => {
    ...
    if (armedColor) {
      if (grid[r][c].type === "EMPTY") {
        setUpgradeCoin({ pos: gridToPos(r, c, selectedFeatures), color: armedColor });
        setArmedColor(null);
      }
      return;
    }
  ...
  const handleSpin = () => {
    ...
    // Only new coins in the CURRENTLY unlocked rows reset the spin counter.
    // Must use the live `fUnlock` boundary (not the static ROWS_LOCKED=8
    // threshold that `isPosLocked` uses) — otherwise a coin landing in a row
    // that just unlocked (e.g. row 7) is wrongly treated as still-locked and
    // only decrements the spin counter instead of resetting it. This mirrors
    // the exact fix already applied in the standalone Tower feature.
      ...
      >
        ...
        {isUpgrade && upgradeCoin && (
          <div className="flex flex-col items-center gap-0.5 pointer-events-none">
            ...
            <button
              ...
              className="pointer-events-auto absolute top-0.5 right-1 text-[10px] text-red-400 hover:text-red-200 font-bold leading-none"
            >✕</button>
          </div>
        )}
 
        {/* ⚡ E-Reel button — shown on unlocked cells until one is chosen */}
        {eReelPos === null && unlocked && !isUpgrade && (
          <button
        ...
        {isEmpty && !isUpgrade && (
          <span className="text-gray-600 text-[10px] pointer-events-none">
            {armable ? "place here" : (!unlocked ? "+" : "+ Gold")}
          </span>
        )}
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: "#1e2235" }}>
      ...
      {isOpen && (
        <div className="px-4 pb-5 flex flex-col gap-4">
          ...
          {/* Upgrade coin panel — grow this combo by adding a feature */}
          <UpgradePanel
