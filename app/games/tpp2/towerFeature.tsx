...
export default function TowerFeature({ baseCoins, onSpin, onReset, onUpgrade }: Props) {
  ...
  const upgradeTargets = onUpgrade ? availableUpgradeTargets(["piggyTower"]) : [];
  ...
  const handleCellClick = (r: number, c: number) => {
    ...
    if (armedColor) {
      if (grid[r][c].type === "EMPTY") {
        setUpgradeCoin({ pos: posIdx(r, c), color: armedColor });
        setArmedColor(null);
      }
      return;
    }
                          ...
                          >
                            ...
                            {isUpgrade && upgradeCoin && (
                              <div className="flex flex-col items-center gap-0.5 pointer-events-none">
                                <span className="text-base leading-none">{UP_COLOR_META[upgradeCoin.color].dot}</span>
                                <span className="text-[7px] font-bold text-green-300 leading-none">UPG</span>
                                <button
