...
//   onCoinsChange: (coins: UltraFeatureCoin[]) => void;
//   onSpin:        (line: string) => void;
//   onReset:       () => void;
//   onUpgrade:     (newFeatures: string[], carryCoins: UltraFeatureCoin[]) => void;
// };
...
//   const [coins,     setCoins]     = useState<UltraFeatureCoin[]>(init);
...
//     setCoins(prev => [...prev, { position: pos, colorCode: ULTRA_COIN_COLORS[0].value, value: ULTRA_COIN_VALUES[0], boostValue: "", boostSide: null }]);
//   };
...
//               Double+Ultra: select boost on LEFT or RIGHT per coin — selecting one locks the other. ✕ to clear.
//             </div>
//           )}
...
//                           {ULTRA_COIN_COLORS.map(c => <option key={c.value} value={c.value} className="bg-gray-800">{c.label}</option>)}
//                         </select>
//                         <select className={`text-white text-[9px] w-full rounded px-0.5 py-0.5 border-0 font-mono ${COIN_SELECT_BG[coin.colorCode] ?? "bg-gray-700"}`}
//                           value={coin.value} onClick={e => e.stopPropagation()}
//                           onChange={e => updateCoin(pos, "value", e.target.value)}>
//                           {ULTRA_COIN_VALUES.map(v => <option key={v} value={v} className="bg-gray-800">{v}</option>)}
//                         </select>
//                         {!isDoubleCombo && (
//                           <select className="bg-purple-950 text-purple-200 text-[9px] w-full rounded px-0.5 py-0.5 border-0 font-mono"
//                             value={coin.boostValue || ""} onClick={e => e.stopPropagation()}
...
//                                       className={`text-[8px] w-full rounded px-0 py-0.5 border-0 font-mono ${isThis ? "bg-yellow-700 text-yellow-100" : "bg-purple-950 text-purple-200"} ${locked ? "opacity-30 cursor-not-allowed" : ""}`}
...
//                               })}
//                               {coin.boostSide && (
//                                 <button className="text-red-400 hover:text-red-200 text-[8px] pb-0.5 ml-0.5"
...
type Props = {
  ...
  onSpin:        (line: string) => void;
  onReset:       () => void;
  onUpgrade:     (newFeatures: string[], carryCoins: UltraFeatureCoin[], upgradeInfo: UpgradeInfo) => void;
};
...
export default function UltraFeature({ baseCoins, isDoubleCombo, onCoinsChange, onSpin, onReset, onUpgrade }: Props) {
  ...
  const coinAt    = (pos: number) => coins.find(c => c.position === pos);
  const addCoin   = (pos: number) => { if (coinAt(pos)) return; setCoins(p=>[...p,{position:pos,colorCode:ULTRA_COIN_COLORS[0].value,value:ULTRA_COIN_VALUES[0],boostValue:"",boostSide:null}]); };
  ...
  const updateCoin= (pos: number, field: keyof UltraFeatureCoin, val: any) => setCoins(p=>p.map(c=>c.position===pos?{...c,[field]:val}:c));
  ...
  const upgradeCoinn   = upgradePos !== null ? coinAt(upgradePos) : null;
  const ALLCOLOR_CODE  = ULTRA_COIN_COLORS[ULTRA_COIN_COLORS.length - 1].value;
  ...
  const navigateUpgrade = (feats: string[]) => {
    ...
    onUpgrade([...new Set(["ultra",...feats.map(f=>f.toLowerCase())])], coins, upgradeInfo);
  };
        ...
        <div className="p-4 pt-0 flex flex-col gap-3">
          <div className="flex items-center gap-3 flex-wrap">
            <button onClick={handleSpin} disabled={spinsLeft <= 0}
              className={`px-5 py-1.5 rounded font-bold font-mono transition-all ${spinsLeft>0?"bg-purple-600 hover:bg-purple-500":"bg-gray-600 opacity-50 cursor-not-allowed"}`}>SPIN</button>
            <span className="text-sm text-gray-400 font-mono">{spinsLeft} spin{spinsLeft!==1?"s":""} left</span>
            <button onClick={reset} className="px-3 py-1 bg-gray-600 hover:bg-gray-500 rounded text-sm">Reset</button>
          </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  {upgradeMultiSel.size > 0 && (
                    <button onClick={() => navigateUpgrade(Array.from(upgradeMultiSel))}
                      ...
                      → Go to ultra + {Array.from(upgradeMultiSel).map(f=>f.toLowerCase()).join(" + ")}
                    </button>
                  )}
                </div>
              ) : (
                ...
                <div key={pos} onClick={() => !coin && addCoin(pos)}
                  ...
                  {coin ? (
                    <div className="flex flex-col items-center w-full gap-0.5 mt-0.5">
                      ...
                      {!isDoubleCombo && (
                        <select className="bg-purple-950 text-purple-200 text-[9px] w-full rounded px-0.5 py-0.5 border-0 font-mono"
                          value={coin.boostValue||""} onClick={e=>e.stopPropagation()} onChange={e=>updateCoin(pos,"boostValue",e.target.value)}>
                          <option value="">Boost</option>
                          {ULTRA_BOOST_VALUES.map(v=><option key={v} value={v} className="bg-gray-800">{v}</option>)}
                        </select>
                      )}
