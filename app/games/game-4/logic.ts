...
//   const data: Record<string, any> = {};
//   for (const field of fields) {
//     const value = extractField(grid, field, context);
//     // Skip parameters that resolved to no data (e.g. a coin/multiplier/coinPrize
//     // param where nothing on the grid matched) rather than printing an empty array.
...
//   // whether to include a common, free-text label (e.g. "COIN") after position.
//   // This is a single value typed once on the parameter itself — not per cell —
//   // and applies to every entry this field produces. Optional and independent
//   // of includeValue — either, both, or neither can be on.
//   includeLabel?: boolean;
//   labelText?: string;
//   // when both a value and a label are included, this controls whether they're
//   // combined as "label_value" (default) or "value_label" into a single entry.
//   labelOrder?: "labelFirst" | "valueFirst";
...
//             const hasValue = !!field.includeValue && cell.value !== undefined;
//             const hasLabel = !!field.includeLabel && !!field.labelText;

//             if (hasValue && hasLabel) {
//               const combined = field.labelOrder === "valueFirst"
//                 ? `${cell.value}_${field.labelText}`
//                 : `${field.labelText}_${cell.value}`;
//               entry.push(combined);
//             } else if (hasValue) {
//               entry.push(cell.value);
//             } else if (hasLabel) {
//               entry.push(field.labelText);
//             }
...
// // Like JSON.stringify, but strings are written without quotes — so text
// // labels (e.g. coin_100) appear as raw text in the output instead of "coin_100".
// function stringifyNoQuotes(v: any): string {
//   if (Array.isArray(v)) {
//     return `[${v.map(stringifyNoQuotes).join(",")}]`;
//   }
//   if (typeof v === "string") {
//     return v;
//   }
...
//       .map(([k, val]) => `${k}:${stringifyNoQuotes(val)}`)
//       .join(",")}}`;
//   }
...
export type SchemaFieldType =
  ...
  | "multiplier"  // positions of cells that have a multiplier
  | "coinPrize"   // custom tuple per landed coin — position(s) + prize + tags, in any order
  | "dynamicKey"  // one output key PER matching cell, position baked into the key itself:
                  //   reelstripCOR_32:[MINOR]  — "32" is the position, generated from a
                  //   single "reelstripCOR" parameter rather than a fixed key
  | "static"      // a fixed value
  | "custom";     // raw JS expression evaluated against grid (advanced)
...
export type SchemaField = {
  ...
  // prefix used to build each dynamically-generated key for type === "dynamicKey",
  // e.g. keyPrefix "reelstripCOR" + position 32 → key "reelstripCOR_32"
  keyPrefix?: string;
  // whether to include the cell's symbol/coin type (e.g. "MINOR") in the value —
  // defaults to true, since that's normally the whole point of this field type.
  includeType?: boolean;
...
export function extractField(
  ...
  switch (field.type) {
    ...
    case "dynamicKey": {
      // Unlike other field types, this doesn't return one array under a fixed
      // key — it returns a map of *newly-built* keys (prefix + position),
      // one per matching cell, to be merged directly into the output.
      const map: Record<string, any[]> = {};
      const prefix = field.keyPrefix?.trim() || field.key;
      ...
      for (let j = 0; j < cols; j++) {
        for (let i = 0; i < rows; i++) {
          ...
          const pos = formatPosition(j, i, rows, cols, posFormat);
          const posStr = Array.isArray(pos) ? pos.join("_") : String(pos);
          const key = `${prefix}_${posStr}`;

          const entry: any[] = [];
          if (field.includeType !== false) entry.push(cell.type);
          if (field.includeValue && cell.value !== undefined) entry.push(cell.value);
          if (field.includeLabel && field.labelText) entry.push(field.labelText);
...
export function generateFromSchema(
  ...
  for (const field of fields) {
    ...
    if (field.type === "dynamicKey") {
      // value is a { generatedKey: entry[] } map — merge its keys straight
      // into the output instead of nesting them under field.key.
      if (value && typeof value === "object") {
        for (const [k, v] of Object.entries(value)) data[k] = v;
      }
      continue;
    }
