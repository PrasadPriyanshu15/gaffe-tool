...
// export function OutputPanel({ outputs, currentPreview }: OutputPanelProps) {
...
//         <span className="text-xs font-bold uppercase tracking-widest text-gray-300">Result</span>
//         <button
//           onClick={handleCopyAll}
//           disabled={outputs.length === 0}
//           className="text-[11px] bg-gray-800 hover:bg-gray-700 disabled:opacity-40 disabled:cursor-not-allowed text-gray-300 px-2.5 py-1 rounded-md font-medium transition-colors"
//         >
...
//               <pre
//                 key={i}
//                 className="text-[11px] text-green-300 font-mono whitespace-pre-wrap mb-6 pb-6 border-b border-gray-800 last:border-b-0 last:mb-0 last:pb-0"
//               >
//                 {o}
//               </pre>
//             ))
...
//                         <PositionField field={field} positionOrder={positionOrder} onChange={patch => updateField(index, patch)} />
//                         <Toggle label="+ value" value={!!field.includeValue} onChange={v => updateField(index, { includeValue: v })} />
//                         <Toggle label="+ label" value={!!field.includeLabel} onChange={v => updateField(index, { includeLabel: v })} />
//                         {field.includeLabel && (
//                           <input
//                             value={field.labelText ?? ""}
//                             onChange={e => updateField(index, { labelText: e.target.value })}
//                             placeholder="label text e.g. COIN"
//                             className="bg-gray-900 border border-gray-600 text-cyan-300 text-xs px-2 py-1 rounded font-mono w-28"
//                           />
//                         )}
//                         {field.includeLabel && field.includeValue && (
//                           <button
//                             onClick={() => updateField(index, { labelOrder: field.labelOrder === "valueFirst" ? "labelFirst" : "valueFirst" })}
//                             title="Toggle order of label and value in the combined entry"
//                             className="text-[10px] px-2 py-1 rounded border bg-gray-900 border-gray-600 text-gray-300 hover:border-cyan-400 font-mono whitespace-nowrap"
//                           >
//                             {field.labelOrder === "valueFirst" ? "value_label" : "label_value"}
//                           </button>
//                         )}
//                       </>
//                     )}
...
const FIELD_TYPES: SchemaFieldType[] = ["reelStops", "coin", "coinPrize", "multiplier", "dynamicKey", "static", "custom"];
...
const FIELD_TYPE_LABELS: Record<SchemaFieldType, string> = {
  ...
  coinPrize: "Coin Prize (custom)",
  multiplier: "Multiplier Value",
  dynamicKey: "Keyed by Position (dynamic key)",
  static: "Static Value",
  custom: "Custom Expression",
};
              ...
              {fields.map((field, index) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    ...
                    {field.type === "dynamicKey" && (
                      <>
                        <div className="flex items-center gap-1">
                          <span className="text-gray-500 text-[10px]">key prefix</span>
                          <input
                            value={field.keyPrefix ?? ""}
                            onChange={e => updateField(index, { keyPrefix: e.target.value })}
                            placeholder="e.g. reelstripCOR"
                            className="bg-gray-900 border border-gray-600 text-white text-xs px-2 py-1 rounded font-mono w-32"
                          />
                        </div>
                        <div className="flex items-center gap-1">
                          <span className="text-gray-500 text-[10px]">symbol</span>
                        ...
                        <PositionField field={field} positionOrder={positionOrder} onChange={patch => updateField(index, patch)} />
                        <Toggle label="+ type" value={field.includeType !== false} onChange={v => updateField(index, { includeType: v })} />
                        <Toggle label="+ value" value={!!field.includeValue} onChange={v => updateField(index, { includeValue: v })} />
                        <Toggle label="+ label" value={!!field.includeLabel} onChange={v => updateField(index, { includeLabel: v })} />
                        {field.includeLabel && (
                          <input
                        ...
                        <span className="text-gray-600 text-[10px] w-full">
                          e.g. {field.keyPrefix || field.key}_32:[{field.includeType !== false ? (field.coinType || "SYMBOL") : ""}
                          {field.includeValue ? (field.includeType !== false ? ",100" : "100") : ""}
                          {field.includeLabel && field.labelText ? `${(field.includeType !== false || field.includeValue) ? "," : ""}${field.labelText}` : ""}]
                        </span>
                      </>
                    )}
