...
//                     {coin && config.hasSplit && (coin.splitCount ?? 1) > 1 &&
//                       Array.from({ length: (coin.splitCount ?? 1) - 1 }).map((_, idx) => (
//                         <div key={idx} className="rounded border border-dashed border-pink-400/50 bg-pink-950/30 flex flex-col items-center p-1 min-h-[50px] text-[9px] text-pink-200">
//                           <div className="opacity-40 self-start">split {idx + 2}</div>
//                           <div className="text-sm">🟡</div>
//                           <div className="text-[9px] opacity-60">{coin.value}</div>
//                           {config.hasStrike && coin.winged && (
//                             <select
//                               className="text-white text-[9px] w-full rounded bg-yellow-800 border-0 mt-0.5"
//                               value={coin.splitBoostValues?.[idx] ?? ""}
//                               onClick={(e) => e.stopPropagation()}
//                               onChange={(e) => updateSplitBoost(pos, idx, e.target.value)}
//                             >
//                               <option value="" className="bg-gray-800">Boost</option>
//                               {SPLIT_BOOST_VALUES.map((v) => (
//                                 <option key={v} value={v} className="bg-gray-800">{v}</option>
...
//             {config.hasStrike && <span>🪽🟡🪽 = winged — click to toggle</span>}
//             {config.hasSplit  && <span>Split × N = N copies</span>}
...
//                         const copyVal = coin.splitCopyValues?.[copyIdx] ?? coin.value;
//                         return (
//                           <div key={idx} className="rounded border border-dashed border-pink-400/50 bg-pink-950/30 flex flex-col items-center p-1 min-h-[60px] text-[9px] text-pink-200 gap-0.5">
...
//                           {/* Winged radio (copy 0) — only in Strike combo */}
//                           {config.hasStrike ? (
//                             <div className="flex flex-col items-center w-full gap-0.5">
...
//                           {/* Boost (copy 0) — only shown when copy 0 is the winged copy */}
