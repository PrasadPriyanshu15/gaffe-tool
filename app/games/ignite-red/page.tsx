...
import { ALL_FEATURES, SCAT_COLOR_CODE, ScatType, posToCol, posToRow } from "./config";
import {
  ComboCoin, ComboFeatureConfig,
  getComboScatSeedColor, getComboKey,
...
// Scat keys that carry zone params
const ZONE_SCAT_KEYS  = new Set(["blue", "all"]);
// Scat keys that carry boost (ultra)
const ULTRA_SCAT_KEYS = new Set(["purple", "all"]);
// Scat keys that are red/double
const DOUBLE_SCAT_KEYS = new Set(["red", "all"]);

// ── Helpers ───────────────────────────────────────────────────────────────────

/** Derive zone splitter + multipliers from the base-game scats */
...
/** Build generic base coins (Zone / Extra use these) */
...
/** Build Double base coins (carry leftValue + rightValue from base scats) */
...
/** Build Ultra base coins (carry boostValue from base scats) */
...
/** Build combo base coins (colorCode remapped per combination) */
...
export default function Home() {
  ...
  const [featureKey, setFeatureKey] = useState(0); // bump to force re-mount
  ...
  // Seed coins for the current feature section:
  // after an upgrade, use carryCoins; else use fresh base coins
  ...
  const handleGoTo = (features: string[]) => {
    const g = generateGaffe(reelStops, scats, features, grandEnabled);
  ...
  const handleUpgrade = (newFeatures: string[], carry: any[]) => {
    const g = generateGaffe(reelStops, scats, newFeatures, grandEnabled);
    setBaseGaffeLine(formatGaffe(g));
    // Keep existing history; spin counter resets via featureKey re-mount
    setCarryCoins(carry);
